...
int main() {
  ...
  constexpr auto sigma_3  = 1.;
  constexpr auto sigma_1  = 10. * sigma_3;
  constexpr auto sigma_2  = 2. * sigma_3;
  constexpr auto sigma_12 = ( sigma_1+sigma_2 ) / 2.;
  constexpr auto sigma_13 = ( sigma_1+sigma_3 ) / 2.;
  constexpr auto sigma_32 = ( sigma_3+sigma_2 ) / 2.;

  
  auto r = vector<double>(100);  //Cree un vector de floats de 101 zeros 
//  auto r = vector<double>{5.5}; 
  iota( begin(r), end(r), sigma_12*100. ); //Convertir el vector en {1000,1001,...,1099}
					       

  transform(policy, begin(r), end(r), begin(r), [](auto i){
		  return i/100;}); //Transformar en el vector {10.00,10.01,...,11.00}
 
  ofstream fout("beta_u_dis_ter_tri_s2_2.csv"); // Create/output to a file

  //Funcion a integrar 
  auto f = [sigma_13,sigma_32](auto rho, auto q){
  	return (sph_bessel(0, rho*q ) * sph_bessel(1, q*sigma_13 )  * sph_bessel(1, q*sigma_32 ) ) ;
  };
  
  // Propiedades de la integración
  constexpr auto a         = 0.0001 ;
  constexpr auto b         = 1000.0001;
